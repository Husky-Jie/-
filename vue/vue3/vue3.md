...
路由核心：改变url，但是页面不进行整体刷新，路由可理解为指向

路由表：是一个映射表，一个路由就是一组映射关系，key:value， key 为路径, value 可能是 function 或 component
...
2.在src下创建views文件夹，该文件夹一般放路由渲染的组件（vue文件），创建Home.vue,  About.vue
...
![image-20230630201530185](assets\image-20230630201530185.png)
...
直接在传递给 `router.push` 的 `routeLocation` 中增加一个属性 `replace: true`
...
```js
const routes = [{path: '/user/:id(\\d+)', component: User, name:"user", props: true}]
```
...
[axios中文文档 (javasoho.com)](https://javasoho.com/axios/)
