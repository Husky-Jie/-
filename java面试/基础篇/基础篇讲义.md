...
* ArrayList 是 fail-fast 的典型代表，遍历的同时不能修改，一旦发现遍历的同时有其他人修改，则立即抛出异常（ConcurrentModificationException并发修改异常）
  * ![image-20230313100030612](.\assets\image-20230313100030612.png)
  * 根据ArrayList的内部类迭代器（Itr）源码，一开始会记录修改的次数modCount，迭代器创建时会记录ArrayLIst集合被修改了几次，定义在迭代器的成员变量expectedModCount中
    * ![image-20230313112138022](.\assets\image-20230313112138022.png)
  * 在每次调用next的方法中都会调用checkForComodification()方法，检查迭代器内部一开始记录的expectedModCount跟外部ArrayList记录的修改次数modCount作比较，若相等则不做任何处理，反之，则抛出并发修改异常（ConcurrentModificationException）
* CopyOnWriteArrayList 是 fail-safe 的典型代表，遍历的同时可以修改，原理是读写分离（牺牲一致性）
  * ![image-20230313095628692](.\assets\image-20230313095628692.png)
  * 通过CopyOnWriteArrayList源码中的add()方法，在添加元素时，会将原数组复制一份，并将新数组长度加一，其最后的索引设置为新添加的值，新数组用来写，旧数组用来读
    * ![image-20230313151554438](.\assets\image-20230313151554438.png)

**提示**

* [测试代码见](D:\Java学习\interview\data_structure\src\list\FailFastVsFailSafe.java) ，这里不再列出
...
* 情况2：remove 树节点时，移除之前检查若 root、root.left、root.right、root.left.left 有一个为 null ，也会退化为链表
...
* 再进行调用 HashMap 的 hash() 方法进行二次哈希
  ...
  * 高位数据是指数目很大的那些数据
    * 进行二次哈希进行扰动，哈希分布会更均匀（如下），若不进行二次哈希，则会出现只有0，2下标的现象，分布不均匀
    * ![image-20230414222352425](.\assets\image-20230414222352425.png)
* 最后 & (capacity – 1) 得到索引
  *  & (capacity – 1)相当于hash % 数组容量，但前者效率更高
...
* **容量是 2 的 n 次幂** 这一设计计算索引效率更好，但 hash 的分散性就不好，需要二次 hash 来作为补偿，没有采用这一设计的典型例子是 Hashtable
  * 使用质数当容量，会使 hash 的分散性更为平均（hash % 质数容量值）
  * 使用容量是 2 的 n 次幂，会使计算索引效率更好，优点为上述1、2
  * Hashtable选取一个质数当作初始容量值，并且之后扩容会以旧容量值*2+1进行扩容
...
5. 返回前检查容量是否超过阈值，一旦超过进行扩容
   1. 添加元素时，首先把元素添加到旧数组，在进行扩容，扩容之后再重新计算桶下标，并迁移元素
...
2. 1.7 是大于等于阈值且没有空位时才扩容，而 1.8 是大于阈值就扩容
   1. 没有空位是指桶下标已有一个node，（再添加一个新元素，若大于等于阈值时，只要该元素的桶下标下有node，就会发生扩容）
...
3. 小于这个值，冲突减少了，链表短了，性能增强，但扩容就会更频繁，空间占用也更多
...
1. HashMap 的 key 可以为 null，但 Map 的其他实现则不然，比如HashTable
