...
参考课前资料提供的文档《 [seata的部署和集成.md](./seata的部署和集成/seata的部署和集成.md) 》：
...
public class AccountTCCServiceImpl implements AccountTCCService {
    ...
    public void deduct(String userId, int money) {
        ...
        // 判断freeze中是否有冻结记录，如果有，一定执行过cancel，拒绝业务
    ...
    public boolean cancel(BusinessActionContext ctx) {
        ...
        // 空回滚判断。判断accountFreeze是否为null，为null证明try没有执行，需要空回滚
        // key为try业务重写方法的接口上定义的注解@BusinessActionContextParameter(paramName = "userId")中paramName的值
        String userId1 = ctx.getActionContext("userId").toString();
